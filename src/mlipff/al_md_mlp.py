...
def batch_remove(files):
    for file in files:
        for path in Path().glob(file):
            path.unlink(missing_ok=True)  # unlink removes the file, missing_ok avoids errors if the file doesn't exist

n_cores = 16  # Number of cores used, adjust accordingly

# Cleanup files before starting to ensure no leftover data
batch_remove(["preselected.cfg.*", "preselected.cfg", "selected.cfg", "nbh.cfg", "*.txt"])
...
while True:
    ...
    run_command(f"sbatch -J lammps -o lammps.out -e lammps.err -N 1 -n {n_cores} -p high,MMM ./run_lammps.sh")
    ...
    if n_preselected > 0:
        ...
        run_command("sbatch -J selection -o selection.out -e selection.err -N 1 -n 8 ./select.sh")
        ...
        for i in range(n_orca):
            ...
            orca_dir.mkdir(parents=True, exist_ok=True)  # Create directory if it doesn't exist
        ...
        for i in range(n_orca):
            ...
            os.chdir(Path.cwd().parent.parent)  # Navigate back to the original working directory
        ...
        for i in range(n_orca):
            run_command(f"./mlipff convert ORCA/{i}/OUTCAR VASP/{i}/calculated.cfg --input_format=outcar")
            with Path(f"VASP/{i}/calculated.cfg").open("r") as cfg_file:
                with Path("train.cfg").open("a") as train_file:
                    train_file.write(cfg_file.read())  # Append contents of each calculated.cfg to train.cfg
            #batch_remove([f"ORCA/{i}"])  # Remove the ORCA directory after processing

        # Run training job using sbatch
        is_training_finished = False
        run_command("sbatch -J train -o train.out -e train.err -N 1 --exclusive  ./train.sh --time=3:00:00")
